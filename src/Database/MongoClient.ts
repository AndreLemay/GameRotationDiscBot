...
export const getGames = async () => {
    if (!collections.games)
        throw new Error("Tried to use DB before connection was initialized - call connectToDB first.")
...
export const updateBotConfig = async (config: BotConfig) => {
    ...
    return await collections.config.updateOne({ guildId: config.guildId }, config, { upsert: true })
}
